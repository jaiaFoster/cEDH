...
| 4 | ~100 manually inspected games | **Diagnostic phase started, systematic run not yet begun** | Diagnostic gameplay authorized 2026-08-12. Built a real orchestration adapter (`sim/rules_engine/xmage_adapter/`, `INFRA-0004`, resolved) and ran/reviewed a first batch of 7 real deck-backed AI-vs-AI games (`results/diagnostic/`) - zero rules-correctness defects found across the full defect checklist. Found and fixed one adapter bug (seed timing); found and scoped (not fixed) two engine-AI limitations that don't block progress: `INFRA-0005` (seed doesn't guarantee bit-exact AI replay) and `INFRA-0006` (the engine's default AI is unreliable at 4 seats - **near-term Gate 4 batch should run 2-player Commander Duel**, where the AI substrate is demonstrably reliable). The actual ~100-game systematic manually-inspected run has not started yet. | 2026-08-12 |
