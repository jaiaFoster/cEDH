...
| 4 | ~100 manually inspected games | **`GATE_4A_2P_DIAGNOSTIC` sub-phase complete; canonical Gate 4 NOT passed** | Diagnostic gameplay authorized 2026-08-12. `INFRA-0004` (real orchestration adapter, `sim/rules_engine/xmage_adapter/`) resolved. Ran and reviewed `GATE_4A_2P_DIAGNOSTIC`: 100 real deck-backed 2-player Commander Duel games (`results/diagnostic/gate4a_2p/`, seeds 1-100, exact frozen deck+hash recorded per game) - 100/100 completed, 0 exceptions/errors/hangs, zero `RULES_DEFECT`/`ADAPTER_DEFECT`/`TRANSCRIPT_DEFECT` found (`SIM-0016`). One systemic `XMAGE_AI_POLICY_LIMITATION` found and explicitly left unfixed per instruction (`INFRA-0007`, Pact of Negation - explains all 5 detected wins, why win data here is not strategic evidence). Investigated `INFRA-0006` (4-player AI passivity): skill-parameter tuning (`skill=10`) gives a real, free activity improvement, verified at a 20-game probe (`results/diagnostic/gate4a_4p_probe/`) - adopted as the 4-player default; residual per-game passivity documented as a labeled AI-policy fidelity limitation, not a defect. **This sub-phase is explicitly NOT the charter's canonical ~100-manually-inspected-game Gate 4** (that requires the full charter methodology, e.g. real per-game manual inspection notes and eventual multi-archetype opponents) - it is a rules/adapter/policy stress test that establishes the substrate is sound enough to attempt it. | 2026-08-12 |
