...
| 4 | ~100 manually inspected games | **`GATE_4A_2P_DIAGNOSTIC` sub-phase complete (v2, corrected); canonical Gate 4 NOT passed** | Diagnostic gameplay authorized 2026-08-12. `INFRA-0004` (real orchestration adapter) resolved. First pass (`SIM-0016`, 100 games) found zero rules/adapter/transcript defects but was later found to have run with a severe `ADAPTER_DEFECT` (`INFRA-0008`): the test harness's `testMode=true` skips real hand-dealing, so every game played from an empty hand refilled only by draws, never a real 7-card opener. Fixed (`executeWithRealHands()`, `testMode=false` in both game drivers) and rerun at a reduced sample (time-boxed): `GATE_4A_2P_DIAGNOSTIC` v2, 40 games (`results/diagnostic/gate4a_2p_v2/`), activity roughly doubled (50.6 -> 102.6 events/game), 0 new defects. `INFRA-0007` (Pact of Negation AI blind spot, explains all detected wins) confirmed to persist under real hands. Four-player probe v2 (8/10 completed, `results/diagnostic/gate4a_4p_probe_v2/`) shows much more balanced per-seat activity than the original `INFRA-0006` probe. **This sub-phase is explicitly NOT the charter's canonical ~100-manually-inspected-game Gate 4** - it is a rules/adapter/policy stress test establishing the substrate (now with real hands) is sound enough to attempt it, at a smaller sample than originally planned pending a follow-up full-scale run. See also `results/solo_baseline/README.md` (SIM-001 SOLO BASELINE v1, and SIM-001 SOLO-002R - a native `DECK_BACKED_GOLDFISH` Turn 1-3 opening-hand/mulligan audit, 100k+ hands, built in a later phase of the same effort). **SOLO-002's initial numbers were superseded by a correctness repair (SOLO-002R) after a critical mana-tapping defect was found; only SOLO-002R's figures are trustworthy** - see that section's write-up and its `test_opening_hand_mana_correctness.py` regression suite. | 2026-08-13 |
