...
| **Foil** alternate cost | **CORRECTED (see Stage 1 Correction Log below)**: `{2}{U}{U}` instant, "Counter target spell." Alternative cost: discard an Island card **and** another card (two cards total, real card-disadvantage cost — not exile-a-blue-card, no mana-value≤3 restriction, counters ANY spell). |
...
| **Breach + LED + Brain Freeze sequencing** | **CORRECTED (see Stage 1 Correction Log below) — this is a genuine, real infinite combo when Brain Freeze targets the caster, not a fuel-limited value engine.** See `sim/analysis/rogfarm001_breach_loop.py` for the exact deterministic iteration model and `rules_tests/regression/test_rogfarm001_breach_loop.py` for validated state transitions. Summary: each loop iteration consumes 6 graveyard cards (exile) and returns 2 (LED/Brain Freeze themselves), but Brain Freeze's OWN self-mill (omitted from the original Stage 1 draft — the actual model defect) adds `3 × (storm_at_cast + 1)` cards to the target's graveyard. Targeting **self**: net graveyard change per iteration = `6 × (n − 1)` where n is the iteration number starting from a turn with 0 prior spells (iteration 1 breaks exactly even; iteration 2 onward is strictly positive and growing without bound) — with even 1 spell cast earlier that turn, iteration 1 is already net-positive. The loop is self-sustaining/effectively infinite once minimally seeded (6 "other" graveyard cards + LED + Brain Freeze already in the graveyard + Breach resolved), bounded only by the target's library size. Targeting an **opponent** instead: no self-refuel occurs (the mill lands in their graveyard, not the caster's) — net **−6 graveyard cards per iteration**, genuinely fuel-limited, matching the original (now-corrected) draft's math but only for this specific sub-case. The real deployable line is therefore hybrid: prime the loop by self-targeting until storm is large, then redirect the final Brain Freeze cast(s) at an opponent for a storm-scaled kill mill. |
