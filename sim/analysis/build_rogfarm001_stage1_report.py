...
RULES_ASSERTIONS_VERIFIED = [
    ...
    "CORRECTED (was: 'net -4/loop, hard fuel-limited' - omitted Brain Freeze's own self-mill): "
    "Underworld Breach escape = mana cost + exile 3 OTHER graveyard cards (verified, WebSearch). "
    "Shell cost is -6 other-pool cards/iteration (3 for LED's escape + 3 for Brain Freeze's "
    "escape). If Brain Freeze targets the CASTER, its self-mill refuels that same pool: "
    "net_delta_n = 3K + 6*(n-1) (K = spells cast earlier this turn, n = iteration number) - "
    "iteration 1 is an exact breakeven at K=0 (or already net-positive at K>=1), iteration 2+ is "
    "strictly positive and grows without bound -> a genuine self-sustaining/infinite loop once "
    "minimally seeded (LED+BrainFreeze in graveyard, Breach resolved, >=6 other graveyard "
    "cards), matching commanderspellbook.com/edh-combos.com's classification. If Brain Freeze "
    "targets an OPPONENT instead, no refuel occurs and the loop is flatly fuel-limited at exactly "
    "-6/iteration. See sim/analysis/rogfarm001_breach_loop.py (exact deterministic model) and "
    "rules_tests/regression/test_rogfarm001_breach_loop.py (18 state-transition tests). R1 has no "
    "Thassa's Oracle (removed vs. stock), so this loop is R1's ONLY deterministic win via this "
    "combo - a direct opponent-library mill, not a self-mill-then-Oracle instant win.",
    "Lion's Eye Diamond: 'Discard your hand, Sacrifice Lion's Eye Diamond: Add three mana of any "
    "one color. Activate only any time you could cast an instant.' Not a spell cast (no storm "
    "contribution); returns to the graveyard when sacrificed (no exile replacement).",
    "Brain Freeze: 'Target player mills three cards. Storm.' Total resolutions per cast = spells "
    "cast before it this turn, + 1; each resolution mills its own target 3 cards; the original "
    "card returns to the graveyard after resolving (not exiled).",
    ...
    "CORRECTED (was: 'exile a blue card... MV<=3 restriction' - a conflation with an unrelated "
    "budget counterspell): Foil is {2}{U}{U} Instant, 'Counter target spell. Alternative cost: "
    "You may discard an Island card and another card rather than pay this spell's mana cost.' No "
    "mana-value restriction on the target; the alt cost is a genuine 2-card DISCARD (to "
    "graveyard), never an exile, and requires an actual Island-subtype CARD in hand (a basic or "
    "an Island-type dual/fetchable dual - R1 runs zero basic Islands, so its fuel is Underground "
    "Sea/Volcanic Island/Scalding Tarn-fetchable duals) plus one further, unrestricted card. See "
    "sim/analysis/rogfarm001_cards.py and rules_tests/regression/test_rogfarm001_foil.py (8 tests).",
]
